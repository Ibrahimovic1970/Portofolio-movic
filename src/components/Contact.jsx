...
export default function Contact() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setTimeout(() => setStatus({ loading: false, success: false, error: '', mock: false }), 5000);
        } catch (err) {
            setStatus({ loading: false, success: false, error: err.message || 'Gagal mengirim. Coba lagi nanti.', mock: false });
        }
    };
            ...
            <div className="container" style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
                <div ref={formRef} className="reveal-up" style={{ maxWidth: '640px', margin: '0 auto' }}>
                    ...
                    <div className="glass-card" style={{ padding: '40px 32px' }}>
                        {status.success ? (
                            <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                                ...
                                {status.mock && (
                                    <p style={{ fontSize: '0.85rem', color: 'var(--accent-secondary)', background: 'rgba(6,182,212,0.1)', padding: '12px', borderRadius: '8px', marginTop: '16px' }}>ℹ️ Mock Mode: Email tidak dikirim ke server.</p>
                                )}
                            </div>
                        ) : (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', marginTop: '40px' }}>
                        {[
                            { icon: <FiMail size={20} />, href: 'mailto:ahmadibrahim.dev@gmail.com', label: 'Email' },
